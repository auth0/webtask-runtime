...
function compile(code, options, cb) {
    ...
    return Async.waterfall([
        (next) => {
            ...
            logger.log('Your code requires the following modules that are not currently available: [' + missing.join(', ') + '].');
            logger.log('Requiring modules that are not natively supported by the webtask platform will incur additional latency each time a new container is used. If you require low-latency and custom modules, we offer a managed webtask cluster option where you can customize the set of installed modules. Please contact us at support@webtask.io to discuss.');
            logger.log();
            logger.log('Installing: [' + missing.join(', ') + '] please wait...');
            logger.log();
    
            Async.map(missing, options.installModule, (err) => {
                if (err) {
                    console.log('An error was encountered while trying to install missing modules: ' + err.message);
                    console.log('Attempting to run the webtask anyway...');
                    console.log();
                }
        ...
        (next) => {
            ...
            try {
                const func = mod._compile(code, pathname) || mod.exports;
