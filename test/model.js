...
describe('Sandbox programming model', () => {
    ...
    it('exposes secrets using the module.webtask api', done => {
        const code = `
        ...
        Runtime.simulate(code, { logger, secrets, parseBody: true }, (res) => {
            expect(res.statusCode).to.equal(200);
            expect(res.payload).to.equal(JSON.stringify({ secrets }));
